...
import SanskariRobot.modules.sql.global_bans_sql as gban_sql
import SanskariRobot.modules.sql.users_sql as user_sql
from SanskariRobot import DEV_USERS, OWNER_ID, dispatcher
from SanskariRobot.modules.helper_funcs.chat_status import dev_plus
...
def dbcleanup(update: Update, context: CallbackContext):
    ...
    msg.reply_text("Gᴇᴛᴛɪɴɢ ɪɴᴠᴀʟɪᴅ ᴄʜᴀᴛ ᴄᴏᴜɴᴛ ...")
    invalid_chat_count = get_invalid_chats(update, context)

    msg.reply_text("Gᴇᴛᴛɪɴɢ ɪɴᴠᴀʟɪᴅ ɢʙᴀɴɴᴇᴅ ᴄᴏᴜɴᴛ...")
    invalid_gban_count = get_invalid_gban(update, context)

    reply = f"Tᴏᴛᴀʟ ɪɴᴠᴀʟɪᴅ ᴄʜᴀᴛs -  {invalid_chat_count}\n"
    reply += f"Tᴏᴛᴀʟ ɪɴᴠᴀʟɪᴅ ɢʙᴀɴɴᴇᴅ ᴜsᴇʀs - {invalid_gban_count}"

    buttons = [[InlineKeyboardButton("Cʟᴇᴀɴᴜᴘ DB", callback_data="db_cleanup")]]
...
def callback_button(update: Update, context: CallbackContext):
    ...
    if query_type == "db_leave_chat":
        if query.from_user.id in admin_list:
            bot.editMessageText("Lᴇᴀᴠɪɴɢ ᴄʜᴀᴛs ...", chat_id, message.message_id)
            chat_count = get_muted_chats(update, context, True)
            bot.sendMessage(chat_id, f"Lᴇғᴛ  {chat_count} ᴄʜᴀᴛs")
        else:
            query.answer("Yᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀʟʟᴏᴡᴇᴅ ᴛᴏ ᴜsᴇ ᴛʜɪs.")
    elif query_type == "db_cleanup":
        if query.from_user.id in admin_list:
            bot.editMessageText("Cʟᴇᴀɴᴜᴘ ᴜᴘ DB ...", chat_id, message.message_id)
            invalid_chat_count = get_invalid_chats(update, context, True)
            invalid_gban_count = get_invalid_gban(update, context, True)
            reply = "Cʟᴇᴀɴᴇᴅ ᴜᴘ {} ᴄʜᴀᴛs ᴀɴᴅ  {} ɢᴀɴɴᴇᴅ ᴜsᴇʀs ғʀᴏᴍ ᴅʙ.".format(
