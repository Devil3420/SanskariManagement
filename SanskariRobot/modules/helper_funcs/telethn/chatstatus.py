from SanskariRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from SanskariRobot import DRAGONS
...
async def Iron_Men_is_admin(chat_id: int):
    status = False
    Iron_Men = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Iron_Men.id == user.id:
            status = True
            break
    return status
