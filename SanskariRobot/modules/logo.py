...
from SanskariRobot import OWNER_ID
from SanskariRobot.events import register
from SanskariRobot import telethn
...
async def lego(event):
    ...
    try:
        ...
        fillcolor = "black"
        shadowcolor = "blue"
        fnt = glob.glob("./SanskariRobot/resources/fonts/*")
        ...
        await telethn.send_file(
            event.chat_id, file=fname, caption=f"ʟᴏɢᴏ ɢᴇɴᴇʀᴀᴛᴇᴅ ʙʏ 「 𝐒𝐀𝐍𝐒𝐊𝐀𝐑𝐈 𒆜 ʀᴏʙᴏᴛ  」"
        )
        await pesan.delete()
        if os.path.exists(fname):
            os.remove(fname)
    except Exception as e:
        await event.reply(f"ғʟᴏᴏᴅᴡᴀɪᴛ ᴇʀʀᴏʀ, ʀᴇᴩᴏʀᴛ ᴛʜɪs ᴀᴛ @@SANSKARI_CHATTERBOX")
