...
from SanskariRobot import SUPPORT_CHAT, WALL_API, dispatcher
...
def wall(update: Update, context: CallbackContext):
    ...
    if not query:
        ...
        if not json_rep.get("success"):
            msg.reply_text(f"Aɴ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀʀᴇᴅ! Rᴇᴘᴏʀᴛ ᴛʜɪs @Iron_Men_bots ")
        else:
            wallpapers = json_rep.get("wallpapers")
            if not wallpapers:
                msg.reply_text("Nᴏ ʀᴇsᴜʟᴛs ғᴏᴜɴᴅ! Rᴇғɪɴᴇ ʏᴏᴜʀ sᴇᴀʀᴄʜ.")
                return
            else:
